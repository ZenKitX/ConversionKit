...
void benchmarkBasicConversion() {
  ...
  for (final conv in conversions) {
    ...
    print(
      '  ${conv['category']}: ${avgTime.toStringAsFixed(2)} μs/op ($iterations ops)',
    );
  }
  print('');
}
...
void benchmarkTemperatureConversion() {
  ...
  for (final conv in conversions) {
    ...
    final avgTime = stopwatch.elapsedMicroseconds / iterations;
    print(
      '  ${conv['from']} → ${conv['to']}: ${avgTime.toStringAsFixed(2)} μs/op ($iterations ops)',
    );
  }
  print('');
}
...
void benchmarkFormatting() {
  ...
  final converter = UnitConverter();
  final values = [123.456, 123.456000, 0.00001, 1234567.89, 0.0];
...
void benchmarkDataRetrieval() {
  ...
  stopwatch.stop();
  print(
    '  getAllCategories: ${(stopwatch.elapsedMicroseconds / iterations1).toStringAsFixed(2)} μs/op ($iterations1 ops)',
  );
  ...
  stopwatch.stop();
  print(
    '  getCategory: ${(stopwatch.elapsedMicroseconds / iterations2).toStringAsFixed(2)} μs/op ($iterations2 ops)',
  );
  ...
  stopwatch.stop();
  print(
    '  getUnits: ${(stopwatch.elapsedMicroseconds / iterations3).toStringAsFixed(2)} μs/op ($iterations3 ops)',
  );
